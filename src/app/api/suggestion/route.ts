...
const suggestionSchema = z.object({
    suggestion: z
        .string()
        .describe(
            "The code to insert at the current cursor position, or an empty string if no suggestion is needed."
        ),
});
    ...
    try {
        ...
        const prompt = SUGGESTION_PROMPT.replace("{fileName}", fileName)
            ...
            const { output } = await generateText({
                model: anthropic("claude-haiku-4-5-20251001"), // TODO: switch to google gemini once the SDK supports streaming responses
