...
export const processMessage = inngest.createFunction(
    {
        ...
        onFailure: async ({ event, step, }) => {
            ...
            if (internalKey) {
                await step.run("update_message-on-failure", async () => {
                    await convex.mutation(api.system.updateMessageContent, {
                        internalKey,
                        messageId,
                        content: "My apologies, Failed to process this message.",
                    });
                });
            }
        },
    ...
    async ({ event, step }) => {
        ...
        // Create a coding agent with file tools
        ...
        const network = createNetwork({
            ...
            router: ({ network }) => {
                ...
                // Anthropic models can return text and tool calls in the same response, so we want to route to the same agent until there are no more tool calls to process
        ...
        let assistantResponse = "I processed your request. Let me know if you have any other questions!";
       ...
       // Update the message with the assistant's response (This also sets status to completed)
